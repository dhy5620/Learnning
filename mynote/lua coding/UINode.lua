...
function UINode:Load()
    ...
    assert(self.viewCfg.prefabPath or self.viewCfg.prefabPoolName,
           "has no assign prefabPath or prefabPoolName field")
...
function UINode:UpdateTransform()
    ...
    if self.__cacheLocalPos then
        UI.SetLocalPositionXYZ(self.transform, self.__cacheLocalPos.x,
                               self.__cacheLocalPos.y,
                               self.__cacheLocalPos.z or 0)
        self.__cacheLocalPos = nil
    elseif self.__cachePos then
        UI.SetPositionXYZ(self.transform, self.__cachePos.x, self.__cachePos.y,
                          self.__cachePos.z or 0)
        self.__cachePos = nil
    else
...
function UINode:Init(gameObject)
    ...
    if self.isNeedUpdateOnLoad then self:OnUpdate() end
end

function UINode:Unload() self:Destroy() end

function UINode:OnDestroy()
    self.isLoaded = nil
    self.destroyed = true
    if self.attachNode then self:Detach() end
    ...
    if self.bindEventInfos then
        for k, v in pairs(self.bindEventInfos) do
            if v[1] and v[2] then v[1]:UnBind(v[2]) end
        end
        self.bindEventInfos = nil
    end
    if self.viewCfg and self.viewCfg.loadedComponents then
        for i, v in ipairs(self.viewCfg.loadedComponents) do v:OnClose() end
...
function UINode:BindEvent(bindDispather, eventName, handleFunc)
    ...
    self.bindEventInfos[eventName .. tostring(bindDispather)] =
        {bindDispather, eventID}
    return eventID
end
...
function UINode:SetCanvas(canvasName)
    ...
    if self.isLoaded then UIMgr:AddToCanvas(self, self.viewCfg.canvasName) end
end
...
function UINode:AddUIComponent(component, arge)
    ...
    if self.isLoaded then new_comp:OnLoad() end
...
function UINode:InitComponents()
    ...
    if self.viewCfg.isShowBackground then self:AddUIComponent(UI.Background) end
    ...
    if self.viewCfg and self.viewCfg.loadedComponents then
        for i, v in ipairs(self.viewCfg.loadedComponents) do v:OnLoad() end
    end
end
...
function UINode:Hide() self.transform:SetParent(PrefabPool.hide_container) end

function UINode:SetUnloadCallBack(callBack) self.unloadCallBack = callBack end
